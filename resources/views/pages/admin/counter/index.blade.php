        ...
        <table class="table table-bordered align-middle" id="table-same">
            ...
            <tbody>
                ...
                <tr class="text-center">
                    ...
                    <td>{{ $counter->district_id }}</td>
                    <td>{{ $counter->distance }}</td>
                    <td>
                        <a href="{{ route('admin.counters.show', $counter->id) }}" class="btn btn-info btn-sm">View</a>
                        <a href="{{ route('admin.counters.edit', $counter->id) }}" class="btn btn-warning btn-sm">Edit</a>
                        <form action="{{ route('admin.counters.destroy', $counter->id, $location->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
